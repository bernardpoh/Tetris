...
checkPieceTimer g@(GameState {..}) =
  case pieceState of
    Falling piece n
      | n >= fallDelay -> fall fallAmount (g {pieceState = Falling piece 0})
      | otherwise -> g
    Locking piece n
      | n >= lockDelay -> landPiece $ g {grid = putPiece piece grid}
      | otherwise -> g
    Waiting n
      | n >= waitDelay -> nextPiece g
      | otherwise -> g
    ClearingLines n
      | n >= waitDelay -> nextPiece $ g {grid = clearLines grid}
      | otherwise -> g
